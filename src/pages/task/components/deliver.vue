  ...
  >
    <view class="scroll-view-wrapper">
      <view class="task-card" v-for="(item, index) in taskListArr" :key="index">
        <view :url="`/subpkg_task/detail/index?jobId=${item.id}`" @click="godetail(item.id)">
          <view class="header">
        ...
        <view class="footer">
          ...
          <view v-if="item.enablePickUp" class="action" style="background-color: #ef4f3f">
            提货
          </view>
          <view v-else disabled class="action disabled"> 提货 </view>
        </view>
      </view>
    </view>
  </scroll-view>
